...
/**
 * Modified by calanti for; 1) New text input method for the Android Soft
 * Keyboard. Enables various native techniques such as swype, auto-correct,
 * voice input, etc, and fixes lots of input problems experienced using the
 * ordinary method. If AndroidTextInputInterface is supplied to constructor and
 * not null, the default input methods are ignored and the new
 * AndroidOnscreenKeyboard is used. This then inputs text natively into a hidden
 * Android EditText, which then updates the specified focused CalTextField
 * (AndroidKeyboardFeedbackInterface) when text is changed. The CalTextField
 * also provides feedback to the EditText, such as text selection, cut, paste,
 * etc, to keep content synced.
 *
 * If AndroidTextInputInterface is null (e.g. from non-Android platform), then
 * *hopefully* default methods are used instead without problems.
 *
 * 2) Slightly modified ClickListener for usability 3) Better handling of
 * drawing selection and cursorPatch 4) Various other required methods.
 * 
 * @author calanti
 */

public class CalTextField extends Widget implements Disableable, AndroidKeyboardFeedbackInterface {
	...
	protected AndroidTextInputInterface androidTextInputInterface;

	public CalTextField(String text, Skin skin) {
		this(text, skin.get(TextFieldStyle.class));
	}

	public CalTextField(String text, Skin skin, String styleName) {
		this(text, skin.get(styleName, TextFieldStyle.class));
	}

	public CalTextField(String text, TextFieldStyle style) {
		setStyle(style);
	...
	/** calanti addition */
	public CalTextField(String text, TextFieldStyle style, AndroidTextInputInterface androidTextInputInterface) {
		this(text, style);
		if (androidTextInputInterface != null) {
			this.androidTextInputInterface = androidTextInputInterface;
			this.keyboard = new AndroidOnscreenKeyboard();
		}
	}

	/**
	 * calanti addition. Requests the Android Keyboard and binds the EditText to
	 * this instance.
	 */
	private class AndroidOnscreenKeyboard implements OnscreenKeyboard {
		@Override
		public void show(boolean visible) {
			if (visible) {
				androidTextInputInterface.requestKeyboard(CalTextField.this);
				updateDisplayText();
			} else
	...
	public void setStyle(TextFieldStyle style) {
		...
		/** calanti addition - seems getLineHeight is more reliable? */
	...
	protected void drawSelection(Drawable selection, Batch batch, BitmapFont font, float x, float y) {
		// selection.draw(batch, x + selectionX + renderOffset + fontOffset, y -
		// textHeight - font.getDescent(), selectionWidth,
		// textHeight);
	...
	protected void drawCursor(Drawable cursorPatch, Batch batch, BitmapFont font, float x, float y) {
		...
		// x + textOffset + glyphPositions.get(cursor) -
		// glyphPositions.get(visibleTextStart) + fontOffset + font.getData().cursorX,
	...
	void updateDisplayText() {
		...
		for (int i = 0; i < textLength; i++) {
			...
			/**
			 * calanti addition - some fonts apparently have these glyphs, causes building
			 * the glyphPositions to break for TextArea
			 */
	...
	/**
	 * Focuses the next If none is found, the keyboard is hidden. Does nothing if
	 * the text field is not in a stage.
	 * 
	...
	/**
	 * If false, methods that change the text will not fire
	 * {@link ChangeListener.ChangeEvent}, the event will be fired only when user
	 * changes the text.
	 */
	...
	public void setSelection(int selectionStart, int selectionEnd) {
		...
		cursor = selectionEnd;
		/** calanti addition - mimic selection in EditText */
		if (androidTextInputInterface != null)
			androidTextInputInterface.setSelection(selectionStart, selectionEnd);
	}
	...
	public void clearSelection() {
		...
		/** calanti addition - mimic selection in EditText */
		if (androidTextInputInterface != null)
			androidTextInputInterface.setSelection(cursor, cursor);
	...
	public void setCursorPosition(int cursorPosition) {
		...
		/** calanti addition - mimic selection in EditText */
		if (androidTextInputInterface != null)
			androidTextInputInterface.setCursorPosition(cursor);
	}
	...
	/** calanti addition, feedback from Android EditText. */
	@Override
	public void textChanged(CharSequence text, int cursorPosition, int selectionEnd) {

		if (text.length() > 0 && text.charAt(0) == ENTER_ANDROID) {
			androidTextInputInterface.setText(text.subSequence(1, text.length()), cursorPosition - 1);
			return;
		}
	...
	static public interface TextFieldListener {
		public void keyTyped(CalTextField textField, char c);
	}
	...
	static public interface TextFieldFilter {
		public boolean acceptChar(CalTextField textField, char c);
	...
	/**
	 * The default {@link OnscreenKeyboard} used by all {@link CalTextField}
	...
	public class TextFieldClickListener extends ClickListener {
		...
		public void clicked(InputEvent event, float x, float y) {
			if (!disabled)
				if (!isFocused()) {
					Stage stage = getStage();
					if (stage != null)
						stage.setKeyboardFocus(CalTextField.this);
					keyboard.show(true);
		...
		public void touchDragged(InputEvent event, float x, float y, int pointer) {
			...
			setCursorPosition(x, y);
			if (androidTextInputInterface != null)
				androidTextInputInterface.setSelection(selectionStart, cursor);
		}

		public void clickedOriginal(InputEvent event, float x, float y) {
			int count = getTapCount() % 4;
			if (count == 0)
				clearSelection();
			if (count == 2) {
		...
		public boolean touchDownOriginal(InputEvent event, float x, float y, int pointer, int button) {
			if (!super.touchDown(event, x, y, pointer, button))
				return false;
			if (pointer == 0 && button != 0)
				return false;
		...
		public void touchDraggedOriginal(InputEvent event, float x, float y, int pointer) {
			super.touchDragged(event, x, y, pointer);
			setCursorPosition(x, y);
		}
		...
		public boolean keyDown(InputEvent event, int keycode) {
			...
			Stage stage = getStage();
			if (stage == null || stage.getKeyboardFocus() != CalTextField.this)
				return false;
		...
		public boolean keyTyped(InputEvent event, char character) {
			...
			if ((character == TAB || character == ENTER_ANDROID) && focusTraversal) {
				...
				if (add || remove) {
					...
					if (add && !remove) {
						...
						if (!enter && filter != null && !filter.acceptChar(CalTextField.this, character))
							return true;
			...
			if (listener != null)
				listener.keyTyped(CalTextField.this, character);
			return true;
		}
	}

	/**
	 * The style for a text field, see {@link CalTextField}.
	 * 
	 * @author mzechner
	 * @author Nathan Sweet
	 */
	static public class TextFieldStyle {
		...
		/** calanti addition */
		public boolean androidKeyboardNumericalOnly;
		public boolean androidKeyboardAutoCorrect;
		public boolean androidKeyboardTextSuggestions;
		...
		public TextFieldStyle(TextFieldStyle style) {
			...
			this.androidKeyboardAutoCorrect = style.androidKeyboardAutoCorrect;
			this.androidKeyboardNumericalOnly = style.androidKeyboardNumericalOnly;
			this.androidKeyboardTextSuggestions = style.androidKeyboardTextSuggestions;
		}
	}
