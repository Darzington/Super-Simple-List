package com.mygdx.supersimplelist.androidnativekeyboardinputtest.libgdxModified_1_9_3;
...
/**
 * Modified by calanti for: 1) Extend CalTextField for native-like Android text
 * input 2) Added some new features such as auto-sizing with new lines and
 * limiting the number of lines (for CalTextArea$AndroidOnscreenKeyboard only)
...
public class CalTextArea extends CalTextField {
	...
	public void moveCursorLine(int line) {
		if (line < 0) {
			...
			if (moveOffset < 0) {
				moveOffset = lineMetas.size <= cursorLine ? 0
			...
			while (cursor < text.length() && cursor <= lineMetas.get(cursorLine).endIndex - 1
					&& glyphPositions.get(cursor)
							- glyphPositions.get(lineMetas.get(cursorLine).startIndex) < moveOffset) {
				cursor++;
			}
			showCursor();
		}
	...
	private int calculateCurrentLineIndex(int cursor) {
		...
		for (int i = 0; i < lineMetas.size; i++) {
			if (cursor > lineMetas.get(i).startIndex)
				index++;
			if (cursor > lineMetas.get(i).endIndex)
				index++;
		}
		return index;
	}
		...
		while (i < lineMetas.size && i < (firstLineShowing + linesShowing)) {
			...
			if (!((minIndex < lineStart && minIndex < lineEnd && maxIndex < lineStart && maxIndex < lineEnd)
				...
				if (start < glyphPositions.size && end < glyphPositions.size) {
					...
					float selectionWidth = Math.max(glyphPositions.get(end) - glyphPositions.get(start),
							selection.getMinWidth());
	...
	protected void drawText(Batch batch, BitmapFont font, float x, float y) {
		...
		for (int i = firstLineShowing; i < (firstLineShowing + linesShowing) && i < lineMetas.size; i++) {
			if (lineMetas.get(i).startIndex != lineMetas.get(i).endIndex)
				font.draw(batch, displayText, x, y + offsetY, lineMetas.get(i).startIndex, lineMetas.get(i).endIndex, 0,
						Align.left, false);
			offsetY -= font.getLineHeight();
		}
	}
	...
	protected void moveCursor(boolean forward, boolean jump) {
		...
		// if (index >= 0 && index + 1 < linesBreak.size && linesBreak.items[index] ==
		// cursor && linesBreak.items[index + 1] == cursor) {
		if (index >= 0 && index + 1 < lineMetas.size && lineMetas.get(index).startIndex == cursor
				&& lineMetas.get(index).endIndex == cursor) {
			cursorLine += count;
			if (jump) {
	...
	protected boolean continueCursor(int index, int offset) {
		...
		// return super.continueCursor(index, offset) && (pos < 0 || pos >=
		// linesBreak.size - 2 || (linesBreak.items[pos + 1] != index)
		// || (linesBreak.items[pos + 1] == linesBreak.items[pos + 2]));
		return super.continueCursor(index, offset)
				&& (pos < 0 || pos >= lineMetas.size * 2 - 2 || (lineMetas.get(line).endIndex != index)
